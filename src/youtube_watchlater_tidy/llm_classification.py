    ...
    *,
    ...
    def validate_batch(
        ...
        if response.finish_reason == "length":
            if len(batch) == 1:
                error = RuntimeError(
                    f"provider {provider.name!r} truncated a single-video classification "
