...
class PlaywrightWatchLaterClient:
    """Authenticated Watch Later UI adapter using a persistent Playwright profile.

    The implementation deliberately locates and verifies a playlist row by exact video ID
    before opening its action menu. YouTube DOM/text is not a stable API, so labels and
    scroll bounds are configurable and failures are surfaced rather than guessing.
    """
    ...
    def _matching_row(self, video_id: str):
        # The href substring narrows quickly, then Python verifies the parsed v= parameter.
    ...
    def remove_video(self, video_id: str) -> BrowserRemovalAttempt:
        ...
        actual = _video_id_from_href(href)
        if actual != video_id:
            raise RuntimeError(
        ...
        if menu.count() == 0:
            # Current YouTube playlist rows commonly keep the action menu under #menu.
        ...
        if option.count() == 0:
            # Tolerate the playlist-specific suffix/punctuation while still requiring
            # a removal phrase configured by the user.
        ...
        # Re-check the exact row identity after opening the menu and immediately before
        # the destructive click. Do not trust playlist ordering.
        href = title_anchor.get_attribute("href")
        actual = _video_id_from_href(href)
        if actual != video_id:
            self._page.keyboard.press("Escape")
        ...
        try:
            ...
            # Some YouTube UI variants reuse/diff the row instead of detaching. The exact
            # ID disappearing from the row set is sufficient confirmation.
...
def open_login_session(
    *,
    ...
    """Open a persistent browser profile for the user to sign in without automation."""
    try:
        from playwright.sync_api import sync_playwright
    ...
    *,
    ...
    for item in pending:
        ...
        for attempt_index in range(retries + 1):
            # Re-check the exact decision and move checkpoint immediately before every
            # browser attempt, not merely once when the plan was created.
            if not removal_item_is_authorized(conn, run, item):
                stale_runtime += 1
                final = None
                break
