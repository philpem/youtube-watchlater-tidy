...
def ensure_watchlater_removal_schema(conn: sqlite3.Connection) -> None:
    # playlist_sync tables are intentionally created elsewhere. The removal plan only
    # consults them when a current move decision exists.
    with conn:
...
def _confirmed_move_checkpoint(
    ...
    *,
    ...
    have revalidated it live, which is represented by attempted_at being non-null.
    """
