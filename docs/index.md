...
This is the main operating guide for the implemented workflow. Use the navigation for focused reference pages.
...
Optional documentation site support:
...
When recovery finds an actual archived copy of the video, the HTML review report includes **Recovered video** links to the archive service so you can inspect/watch that copy during manual review. Metadata-only recovery sources are not labelled as playable video.
...
- [`docs/rich-metadata.md`](rich-metadata.md) — selective descriptions/metadata.
